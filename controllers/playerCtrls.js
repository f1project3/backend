...
const getPlayer = (req, res) => {
  db.Player.find({}).then((foundPlayer) => {
    if (!foundPlayer) {
...
const createPlayer = (req, res) => {
  db.Player.create(req.body).then((createdPlayer) => {
    if (!createdPlayer) {
...
const updatedPlayer = (req, res) => {
  db.Player.findByIdAndUpdate(req.params.id, req.body, { new: true }).then(
    (updatedPlayer) => {
      if (!updatedPlayer) {
        res.status(404).json({ message: "Cannot create Player" });
      } else {
...
const deletedPlayer = (req, res) => {
  db.Player.findByIdAndDelete(req.params.id).then((deletedPlayer) => {
    if (!deletedPlayer) {
      res.status(404).json({ message: "Cannot create Player" });
    } else {
