...
const createStats = (req, res) => {
  db.Stats.create(req.body).then((createdStats) => {
    if (!createdStats) {
      res.status(404).json({ message: "Cannot find Stats" });
    } else {
...
const updateStats = (req, res) => {
  db.Stats.findByIdAndUpdate(req.params.id, req.body, { new: true }).then(
    (updatedStats) => {
      if (!updatedStats) {
...
const deleteStats = (req, res) => {
  db.Stats.findByIdAndDelete(req.params.id).then((deletedStats) => {
    if (!deletedStats) {
